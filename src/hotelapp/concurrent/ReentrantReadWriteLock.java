...
public final class ReentrantReadWriteLock {
    ...
    /**
     * Non-blocking method that attempts to acquire the read lock. Returns true if successful. Checks conditions
     * (whether it can acquire the read lock), and if they are true, updates readers info.
     * <p>
     * Note that if conditions are false (can not acquire the read lock at the moment), this method does NOT wait, just
     * returns false.
     */
    public synchronized boolean tryAcquiringReadLock() {
        if (writeLocks.isEmpty() || (writeLocks.size() == 1 && isWriteLockHeldByCurrentThread())) {
            if (isReadLockHeldByCurrentThread()) {
                readLocks.put(Thread.currentThread().getId(), readLocks.get(Thread.currentThread().getId()) + 1);
            } else
    ...
    public synchronized boolean tryAcquiringWriteLock() {
        if (((writeLocks.size() == 1 && isWriteLockHeldByCurrentThread()) || writeLocks.isEmpty()) && readLocks.isEmpty()) {
            if (isWriteLockHeldByCurrentThread()) {
                writeLocks.put(Thread.currentThread().getId(), writeLocks.get(Thread.currentThread().getId()) + 1);
                return true;
            } else {
    ...
    /**
     * Blocking method that will return only when the read lock has been acquired. Calls tryAcquiringReadLock, and as
     * long as it returns false, waits. Catches InterruptedException.
     */
    public synchronized void lockRead() {
        while (!tryAcquiringReadLock()) {
            try {
                ...
                log.error("Exception while running the LockRead: " + e);
            }
        }
    }

    /**
     * Releases the read lock held by the calling thread. Other threads might still be holding read locks. If no more
     * readers after unlocking, calls notifyAll().
     */
    ...
    /**
     * Blocking method that will return only when the write lock has been acquired. Calls tryAcquiringWriteLock, and as
     * long as it returns false, waits. Catches InterruptedException.
     */
    public synchronized void lockWrite() {
        while (!tryAcquiringWriteLock()) {
            try {
                ...
                log.error("Exception while running the WriteLock: " + e);
            }
        }
    ...
    /**
     * Releases the write lock held by the calling thread. The calling thread may continue to hold a read lock. If the
     * number of writers becomes 0, calls notifyAll.
     */
